...
public class Huffman {
	...
	private static void buildCode(Map<Character, String> codeMap, Node node, String t) {
		if (node.isLeaf()) {
			codeMap.put(node.ch, t);
		} else {
			buildCode(codeMap, node.left, t + 0);
			buildCode(codeMap, node.right, t + '1');
		}
	}
	...
	public static void main(String[] args) throws IOException {
		String s=HuffmanReadFile.loadFile(new File("./huffman/11-0.txt"));
		Map<Character, Integer> freq = computeFreq(s);
		Node root = buildTree(freq);
		Map<Character, String> code= buildCode(root);
		String compress = compress(s, code);
		HuffmanWriteFile.write("./huffman/fichier_compresse",compress);
